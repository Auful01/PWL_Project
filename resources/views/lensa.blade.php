    ...
    <tbody>
        ...
        @foreach ($lensa as $b)
            <tr>
                ...
                <td>
                    <button class="btn btn-primary btn-modal-editLensa"  data-bs-toggle="modal" data-bs-target="#editlensaModal" data-kode="{{ $b->kode }} " data-tipe="{{ $b->tipe }}" data-merek="{{$b->merek->id_merek}}" data-gambar="{{' storage/'. $b->gambar}}" data-deskripsi="{{$b->deskripsi}}" data-harga="@currency($b->harga_sewa)" data-url="{{route('lensa.update', $b->kode)}}">Edit</button>
                    <form action="{{ route('lensa.destroy',$b->kode) }}" method="POST">
                    {{-- <a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#editlensaModal" href="{{route('lensa.index',$b->kode)}}">Edit</a> --}}
                    {{-- <button type="button" class="btn btn-success" id="editlensaModal" data-item-id="{{$b->kode}}">edit</button> --}}
                        @csrf
                        @method('DELETE')
...
<div class="modal fade" id="createlensaModal" tabindex="-1" aria-labelledby="createlensaModal" aria-hidden="true">
  ...
  {{-- MODAL EDIT --}}
  <div class="d-flex justify-content-center mt-2 mb-5 navbar-light">
    <div class="modal fade" id="editlensaModal" tabindex="-1" aria-labelledby="createlensaModal" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
                <img class="navbar-brand-icon" src="{{asset('images/stack-logo-blue.svg')}}" width="25" alt="FlowDash">
